...
public class LogInputAspect {
  ...
  public void logInput(JoinPoint joinPoint) {
    ...
    StandardLevel standardLevel = annotation.logLevel();
    if (standardLevel == null) {
      standardLevel = ConfigHolder.INSTANCE.getConfig().defaultLogLevel();
    }
    Level level = Level.valueOf(standardLevel.name());
